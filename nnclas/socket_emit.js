function socket_emit() {
  ...
  socket.on('get_bbox', function(tmpData) {
    ...
    var feat = d3.select("." + d3.select("#areaList").attr("data-sel") + ".selected")[0][0] 
    var tmpBBox = d3.select(feat).node().getBBox();
    curBBoxPix = "&WIDTH=" + tmpBBox.width + "&HEIGHT=" + tmpBBox.height;
    //var LLWeb = L.Projection.Mercator.project({"lat":bbox[1], "lng": bbox[0]});
  ...
  socket.on('clip_points', function(clipData) {
    if(clipData.code == 200) {
  ...
  socket.on('clip_raster', function(clipData) {
    if(clipData.code == 200) {
      d3.select("#dlLinkBtnA").attr("href", clipData.zip);
      d3.select("#dlLinkBtn").classed("disabled", false);
    }
